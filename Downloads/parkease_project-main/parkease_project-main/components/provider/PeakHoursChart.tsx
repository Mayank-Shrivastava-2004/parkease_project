    ...
    return (
        <View className="bg-white rounded-3xl p-5 shadow-sm shadow-gray-200 border border-gray-100">
            <View className="flex-row justify-between items-center mb-4">
                <View>
                    <Text className="text-dark-900 font-bold text-lg">Peak Hours</Text>
                    <Text className="text-gray-400 text-xs mt-1">Busiest times today</Text>
                </View>
                <View className="bg-orange-50 px-3 py-1.5 rounded-xl">
                    <Text className="text-orange-600 font-bold text-xs">Today</Text>
                </View>
            </View>

            <View className="gap-3">
                {data.map((item, index) => {
                    const isHighest = item.bookings === maxBookings;
                    return (
                        <Animated.View
                            key={index}
                            entering={FadeInRight.delay(index * 100).springify()}
                            className={`rounded-2xl p-4 ${isHighest ? 'bg-indigo-50' : 'bg-gray-50'}`}
                        >
                            <View className="flex-row justify-between items-center mb-3">
                                <View className="flex-row items-center">
                                    <View className={`w-10 h-10 rounded-xl justify-center items-center mr-3 ${isHighest ? 'bg-indigo-500' : 'bg-gray-300'}`}>
                                        <Ionicons name="time" size={20} color="white" />
                                    </View>
                                    <View>
                                        <Text className="text-dark-900 font-bold text-base">{item.hour}</Text>
                                        <Text className="text-gray-400 text-xs">{item.bookings} bookings</Text>
                                    </View>
                                </View>
                                <View className="items-end">
                                    <Text className={`font-black text-xl ${isHighest ? 'text-indigo-600' : 'text-gray-600'}`}>
                                        {item.percentage}%
                                    </Text>
                                    {isHighest && (
                                        <View className="bg-indigo-500 px-2 py-0.5 rounded-full mt-1">
                                            <Text className="text-white text-[8px] font-bold uppercase">Peak</Text>
                                        </View>
                                    )}
                                </View>
                            </View>
                            <View className="h-2 bg-white rounded-full overflow-hidden">
                                <Animated.View
                                    ...
                                    style={{
                                        ...
                                        backgroundColor: isHighest ? '#5B5FEF' : '#B2BEC3'
                                    }}
                                />
                            </View>
                        </Animated.View>
